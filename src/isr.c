...
const _rom char game_code[4] _at(0x21AC) = "CT";
const _rom char game_title[12] _at(0x21B0) = "Controller T";

uint8_t pressed_power_count = 0;
...
void _interrupt(0) key_power_irq(void)
{
    ...
    if (KEY_PAD & KEY_POWER)
        return;

    pressed_power_count++;

    if ((pressed_power_count % 3) == 0)
    {
        _int(0x48);
    }
}
